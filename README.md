...
3. **Add Environments variables to the project**
   ...
   ```shell
      ...
      PDC_STRAPI_API_TOKEN="" # kennisbank-api
      KENNIS_BANK_API_PORT="" # kennisbank-api
      KENNIS_BANK_CORS="" # kennisbank-api Example value for more then one domain 'http://localhost:3000, http://localhost:3001'
   ```
...
| PDC_STRAPI_API_TOKEN        | To generate an API token, navigate to the PDC Strapi dashboard and go to**Settings > API Tokens > Create New API Token** . Ensure that only the product API is allowed by selecting the checkboxes for `find` and `findOne`. Once done, click the **Save** button.                                                 | Straing                       |               | Kennisbank-api              |                                                                                                                                                                                                                          |
| KENNIS_BANK_API_PORT        |                                                                                                                                                                                                                                                                                                                    | Number                        |               | Kennisbank-api              |                                                                                                                                                                                                                          |
| KENNIS_BANK_CORS            | This environment variable manages CORS (Cross-Origin Resource Sharing), allowing the server to specify permitted external origins for resource loading. For multiple domains, separate them with a comma (e.g., '[http://localhost:3000](http://localhost:3000), [http://localhost:3001](http://localhost:3001)'). | URL                           |               | Kennisbank-api              |                                                                                                                                                                                                                          |
