...
describe('Objects controller', () => {
  ...
  describe('GET /api/objects', () => {
    ...
    it('should return kennisartikel objects when type is kennisartikel', async () => {
      ...
      const response = await request(app).get(
        `/api/v1/objects?type=${encodeURIComponent('http://localhost:4001/api/v1/objecttypes/kennisartikel')}`,
      );
    ...
    it('should return kennisartikel objects when type is vac', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get(
        `/api/v1/objects?type=${encodeURIComponent('http://localhost:4001/api/v1/objecttypes/vac')}`,
      );
      expect(response.status).toBe(200);
      expect(response.ok).toBe(true);
    ...
    it('should return 400 when type is not an encoded URL', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get('/api/v1/objects?type=http://localhost:4001/api/v1/objecttypes/vac');
      expect(response.status).toBe(400);
      expect(response.ok).toBe(false);
    });
    it('should return 400 when type is empty', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get('/api/v1/objects?type=');
      expect(response.status).toBe(400);
      expect(response.ok).toBe(false);
    });
    it('should return 400 when type is not a valid URL', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get('/api/v1/objects?type=invalid');
      expect(response.status).toBe(400);
      expect(response.ok).toBe(false);
    });
  ...
  describe('GET /api/objects/:id', () => {
    it('should return 200 and kennisartikel object when uuid is valid', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get('/api/v1/objects/a9058a3e-6dd9-480c-a074-e38026bd4ffd');
      expect(response.status).toBe(200);
      expect(response.ok).toBe(true);
    ...
    it('should return 200 and vac object when uuid is valid', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get('/api/v1/objects/a1b2c3d4-e5f6-7g8h-9i0j-k1l2m3n4o5p6');
      expect(response.status).toBe(200);
      expect(response.ok).toBe(true);
    ...
    it('should return 404 when id is not found', async () => {
      fetchMock.mockResponseOnce(JSON.stringify(getStrapiKennisartikelData()));
      const response = await request(app).get('/api/v1/objects/not-exist-uuid');
