 ...
 * });
 */
export const getPathAndSearchParams = ({ queryParams, segments, locale, translations }: GetPathAndSearchParams) => {
  const uniqueSegments = [...new Set(segments)].map((segment) => {
    if (typeof translations !== 'undefined') {
      return translations(segment as string, {
 ...
 * @returns {URL|null} The built URL object, or null if the URL is invalid.
 * @example
 * const { t } = await useTranslation(locale, ['common']);
 * const url = buildURL({
 *   queryParams: { q: 'test' },
 ...
 * });
 */
export const buildURL = ({ queryParams, isOrigin = true, locale, segments, env, key, translations }: BuildURLProps) => {
