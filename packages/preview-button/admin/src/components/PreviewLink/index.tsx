...
import { ContentBlock } from '../../../../global';
...
interface ProductData {
  ...
  price?: { price?: PriceType[] };
  title?: string;
  internalFieldData?: InternalFieldData;
  additional_information?: AdditionalInformation;
}
interface State {
  additionalInformationData: additionalInformationData | null;
  vacData: { mergedContent?: ContentBlock[]; title?: string } | null;
  internalFieldData: InternalFieldData | null;
...
const PreviewLink = () => {
  ...
  const isPreviewSupported = contentTypes.find((type: { uid: string }) => type.uid === data?.layout?.uid);
  ...
  useEffect(() => {
    ...
    const fetchProductData = async () => {
      try {
        ...
        const productData = (response.data as { data: State['productData'] }).data;
  ...
  useEffect(() => {
    ...
    const fetchAdditionalInfo = async () => {
      try {
        ...
        dispatch({ additionalInformationData: (response.data as { data: State['additionalInformationData'] }).data });
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchVacData = async () => {
      try {
        ...
        dispatch({ vacData: (response.data as { data: State['vacData'] }).data });
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchInternalFieldData = async () => {
      try {
        ...
        dispatch({ internalFieldData: (response.data as { data: State['internalFieldData'] }).data });
      } catch (error) {
  ...
  const combinedContent = combineSimilarCategories([
    ...
    ...(Array.isArray(additionalContent) ? additionalContent : []),
  ]);
  ...
  const content = getContentByType({
    ...
    additionalInformation: {
      content: (
        <Content
          data={(data?.initialData?.content?.contentBlock as ContentBlock[]) || []}
    ...
    products: {
      content: (
        <Content
          data={[...combinedContent, productInternalData].filter(Boolean) as ContentBlock[]}
