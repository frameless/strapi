  ...
  switch (type) {
    ...
    case 'h6':
      editor
        .chain()
        .focus()
        .toggleHeading({ level: Number(type.replace('h', '')) })
        .run();
      break;
    case 'paragraph':
      ...
      <Box padding={2} background="neutral100" className="menu-bar">
        <Flex justifyContent="space-between">
          <Flex style={{ flexWrap: 'wrap' }}>
            ...
            <IconContext.Provider value={iconContextColor}>
              <IconButtonGroup className="button-group">
            ...
            <IconButtonGroup className="button-group">
              ...
              {settings.table ? (
                <IconButton
                  ...
                  onClick={() => editor.chain().focus().insertTable({ cols: 3, row: 3, withHeaderRow: true }).run()}
                />
              ) : null}
